...
//											ItemStack itemToAdd = ConfigManager.getConfig().getItemStack(
//													"players." + playerUUID + "." + args[1] + ".items." + items);
//											playerToAdd.getPlayer().getInventory().setItem(itemIndex, itemToAdd);
//										}
									...
									if (sectionAddOffline == null) {
										ConfigManager.getConfig()
												.set("offline_players." + offlinePlayerUUID + ".inv_name", playerUUID);
										ConfigManager.getConfig()
												.set("offline_players." + offlinePlayerUUID + ".death_id", args[1]);
										...
										player.sendMessage(ChatColor.RED
												+ "This player was already going to recieve his inventory!");
									}
								}
							} else {
	...
	public static void fillInv(ConfigurationSection cs, String playerUUID, Player playerToAdd, String death_id) {
		for (String items : cs.getKeys(false)) {
			int itemIndex = parseInt(items);
			ItemStack itemToAdd = ConfigManager.getConfig().getItemStack(
					"players." + playerUUID + "." + death_id + ".items." + items);
